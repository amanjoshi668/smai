...
class Dataset(object):
    def __init__(self, X, y, batch_size, shuffle=False):
        ...
        assert X.shape[0] == y.shape[
            0], 'Got different numbers of data and labels'
        self.X, self.y = X, y
...
def three_layer_convnet(x, params):
    ...
    ############################################################################
    z1 = tf.nn.conv2d(x, conv_w1, strides=[1, 1, 1, 1], padding='SAME')
    z1 = tf.nn.bias_add(z1, conv_b1)
    a1 = tf.nn.relu(z1)
    z2 = tf.nn.conv2d(a1, conv_w2, strides=[1, 1, 1, 1], padding='SAME')
    z2 = tf.nn.bias_add(z2, conv_b2)
    a2 = tf.nn.relu(z2)
    a3 = flatten(a2)
    print(a3.shape, fc_w.shape, fc_b.shape)
    scores = tf.matmul(a3, fc_w)
...
class TwoLayerFC(tf.keras.Model):
    def __init__(self, hidden_size, num_classes):
        ...
        self.fc1 = tf.layers.Dense(
            hidden_size, activation=tf.nn.relu, kernel_initializer=initializer)
...
def two_layer_fc_functional(inputs, hidden_size, num_classes):
    ...
    fc1_output = tf.layers.dense(
        flattened_inputs,
        hidden_size,
        activation=tf.nn.relu,
        kernel_initializer=initializer)
    scores = tf.layers.dense(
        fc1_output, num_classes, kernel_initializer=initializer)
    return scores
...
def train_part34(model_init_fn, optimizer_init_fn, num_epochs=1):
    ...
    with tf.device(device):
        ...
        #         global_step = optimizer_init_fn_bun[1]
        ...
        for epoch in range(num_epochs):
            ...
            for x_np, y_np in train_dset:
                ...
                if t % print_every == 0:
                    print('Iteration %d, loss = %.4f' % (t, loss_np))
                    check_accuracy(
                        sess, val_dset, x, scores, is_training=is_training)
                    print()
                t += 1
...
def model_init_fn(inputs, is_training):
    ...
    layers = [
        ...
        tf.layers.Conv2D(
            ...
            bias_initializer=tf.zeros_initializer()),
        tf.layers.Flatten(),
        tf.layers.Dense(units=num_classes, kernel_initializer=initializer),
    ]
    model = tf.keras.Sequential(layers)
    ############################################################################
    #                            END OF YOUR CODE                              #
...
def model_init_fn(inputs, is_training):
    ...
    ############################################################################
    channel_0, channel_1, channel_2, channel_3, channel_4, fc_1, fc_2, num_classes = 32, 64, 128, 256, 512, 4096, 4096, 10
    initializer = tf.variance_scaling_initializer(scale=2.0)
    layers = [
        tf.layers.BatchNormalization(input_shape=(32, 32, 3)),
        #tf.layers.MaxPooling2D(pool_size=(2, 2), strides = (2,2), padding='valid'),
        #tf.layers.Dropout(),
        tf.layers.Conv2D(
            filters=channel_1,
            kernel_size=(3, 3),
...
# - I tried using VGG 16 model.
# - I added BatchNormalisation after every convlutional layer. 
# - Activation function used is leaky relu, while on output layer softmax is used.
# - Using full VGG 16 model was letting to overfitting of data.
# - Hence reduced the number of layers.
# 
...
#     <tr><th>batch_normalization_10 (Batc </th><td>(None, 32, 32, 3)   </td> <td>     12   </td>  </tr>
#     <tr><th>conv2d_11 (Conv2D)           </th><td>(None, 32, 32, 64)  </td> <td>     1792 </td>     </tr>
#     <tr><th>batch_normalization_11 (Batc </th><td>(None, 32, 32, 64)  </td> <td>     256  </td>   </tr>
#     <tr><th>conv2d_12 (Conv2D)           </th><td>(None, 32, 32, 64)  </td> <td>     36928</td>     </tr>
#     <tr><th>batch_normalization_12 (Batc </th><td>(None, 32, 32, 64)  </td> <td>     256  </td>     </tr>
# <tr><th>max_pooling2d_4 (MaxPooling2 </th><td>(None, 16, 16, 64)    </td>   <td> 0         </td></tr>
# <tr><th>dropout_5 (Dropout)          </th><td>(None, 16, 16, 64)    </td>   <td> 0         </td></tr>
# <tr><th>conv2d_13 (Conv2D)           </th><td>(None, 16, 16, 128)   </td>   <td> 73856     </td></tr>
# <tr><th>batch_normalization_13 (Batc</th><td> (None, 16, 16, 128)  </td>   <td>  512      </td></tr>
# <tr><th>conv2d_14 (Conv2D)           </th><td>(None, 16, 16, 128)   </td>   <td> 147584    </td></tr>
# <tr><th>batch_normalization_14 (Batc </th><td>(None, 16, 16, 128)   </td>   <td> 512       </td></tr>
# <tr><th>max_pooling2d_5 (MaxPooling2</th><td> (None, 8, 8, 128)    </td>   <td>  0        </td></tr>
# <tr><th>dropout_6 (Dropout)          </th><td>(None, 8, 8, 128)     </td>   <td> 0         </td></tr>
# <tr><th>conv2d_15 (Conv2D)           </th><td>(None, 8, 8, 256)     </td>   <td> 295168    </td></tr>
# <tr><th>batch_normalization_15 (Batc </th><td>(None, 8, 8, 256)     </td>   <td> 1024      </td></tr>
# <tr><th>conv2d_16 (Conv2D)         </th><td>  (None, 8, 8, 256)   </td>   <td>   590080  </td></tr>
# <tr><th>batch_normalization_16 (Batc</th><td> (None, 8, 8, 256)   </td>   <td>   1024    </td></tr>
# <tr><th>conv2d_17 (Conv2D)          </th><td> (None, 8, 8, 256)    </td>   <td>  590080   </td></tr>
# <tr><th>batch_normalization_17 (Batc </th><td>(None, 8, 8, 256)     </td>   <td> 1024      </td></tr>
# <tr><th>max_pooling2d_6 (MaxPooling2 </th><td>(None, 4, 4, 256)     </td>   <td> 0         </td></tr>
# <tr><th>dropout_7 (Dropout)          </th><td>(None, 4, 4, 256)     </td>   <td> 0         </td></tr>
# <tr><th>conv2d_18 (Conv2D)           </th><td>(None, 4, 4, 512)     </td>   <td> 1180160   </td></tr>
# <tr><th>batch_normalization_18 (Batc </th><td>(None, 4, 4, 512)     </td>   <td> 2048      </td></tr>
# <tr><th>conv2d_19 (Conv2D)           </th><td>(None, 4, 4, 512)     </td>   <td> 2359808   </td></tr>
# <tr><th>conv2d_20 (Conv2D)           </th><td>(None, 4, 4, 512)     </td>   <td> 2359808   </td></tr>
# <tr><th>dropout_8 (Dropout)          </th><td>(None, 4, 4, 512)     </td>   <td> 0         </td></tr>
# <tr><th>flatten_2 (Flatten)          </th><td>(None, 8192)          </td>   <td> 0         </td></tr>
# <tr><th>dense_4 (Dense)              </th><td>(None, 4096)          </td>   <td> 33558528  </td></tr>
# <tr><th>dense_5 (Dense)              </th><td>(None, 4096)          </td>   <td> 16781312  </td></tr>
# <tr><th>dense_6 (Dense)              </th><td>(None, 10)            </td>   <td> 40970     </td></tr>
# =================================================================
# - Total params: 58,022,742
# - Trainable params: 58,019,408
# - Non-trainable params: 3,334
